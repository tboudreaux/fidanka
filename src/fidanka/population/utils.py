...
def inverse_cdf_sample(f=None, y=None, x=None):
    assert (
        f is not None or y is not None
    ), "Either the function or the data must be specified."
    ...
    inverse_cdf = interp.interp1d(
        cdf_y_norm, x, bounds_error=False, fill_value="extrapolate"
    )
    return inverse_cdf
...
def sample_n_masses(
    ...
    mMax: float = 2,
    nNum: int = 100,
    completness: Union[Callable[[float], float], None] = None,
):
    """
